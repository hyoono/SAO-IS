...
export default function SubmitDocumentPage() {
  ...
  return (
    <div className="max-w-2xl mx-auto">
      <h2 className="text-2xl font-semibold text-white">Submit Document</h2>
      <p className="text-sm text-slate-400 mt-1 mb-6">Upload a new document to start workflow processing.</p>
      ...
      {!dtQuery.isLoading && documentTypes.length > 0 && (
        <form onSubmit={handleSubmit} className="space-y-5 rounded-xl border border-white/10 bg-slate-950/40 p-6">
          <div>
            <label htmlFor="document_type_id" className="block text-sm font-medium text-slate-300 mb-2">Document Type</label>
          ...
          <div>
            <label htmlFor="file" className="block text-sm font-medium text-slate-300 mb-2">File</label>
          ...
          <button type="submit" disabled={submitMutation.isPending}
            className="px-5 py-2.5 text-sm font-medium text-white bg-blue-600/80 hover:bg-blue-600 rounded-lg transition-colors cursor-pointer disabled:opacity-50">
      ...
      {!dtQuery.isLoading && documentTypes.length === 0 && !dtQuery.isError && (
        <p className="text-sm text-slate-400">No document types configured yet.</p>
      )}
    </div>
  )
}
