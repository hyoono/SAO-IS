  ...
  return (
    <div className="max-w-5xl mx-auto space-y-5">
      <div className="flex items-center justify-between">
        <div>
          <h2 className="text-2xl font-semibold text-white">Document Types</h2>
          <p className="text-sm text-slate-400 mt-1">Configure document categories and their workflow assignments.</p>
        </div>
      ...
      {(showForm || editing) && (
        <form onSubmit={handleSubmit} className="rounded-xl border border-white/10 bg-slate-950/40 p-5 space-y-4">
          <h3 className="text-sm font-semibold text-white">{editing ? `Edit: ${editing.name}` : 'New Document Type'}</h3>
          <div className="grid gap-4 sm:grid-cols-3">
            <div>
              <label className="block text-xs text-slate-400 mb-1.5">Name</label>
              <input type="text" value={form.name} onChange={e => setForm({ ...form, name: e.target.value })} required className="w-full rounded-lg border border-white/10 bg-slate-950/60 px-3 py-2 text-sm text-white focus:outline-none focus:ring-2 focus:ring-blue-500/40" />
            </div>
            <div>
              <label className="block text-xs text-slate-400 mb-1.5">Workflow</label>
              <select value={form.workflow_template_id} onChange={e => setForm({ ...form, workflow_template_id: e.target.value })} required className="w-full rounded-lg border border-white/10 bg-slate-950/60 px-3 py-2 text-sm text-white focus:outline-none focus:ring-2 focus:ring-blue-500/40">
                <option value="">Select</option>
                {workflows.map(w => <option key={w.id} value={w.id}>{w.name}</option>)}
              </select>
            </div>
            <div>
              <label className="block text-xs text-slate-400 mb-1.5">Expiry (days)</label>
              <input type="number" value={form.expiry_days} onChange={e => setForm({ ...form, expiry_days: e.target.value })} min={1} placeholder="Optional" className="w-full rounded-lg border border-white/10 bg-slate-950/60 px-3 py-2 text-sm text-white focus:outline-none focus:ring-2 focus:ring-blue-500/40" />
            </div>
          </div>
          <button type="submit" disabled={createMut.isPending || updateMut.isPending} className="px-5 py-2 text-xs font-medium text-white bg-blue-600/80 hover:bg-blue-600 rounded-lg cursor-pointer disabled:opacity-50">{editing ? 'Save' : 'Create'}</button>
        </form>
      )}
      {isLoading && <p className="text-sm text-blue-200/80 py-8 text-center">Loading…</p>}
      {!isLoading && items.length === 0 && <div className="py-16 text-center"><p className="text-slate-400">No document types.</p></div>}
      {items.length > 0 && (
          ...
          <table className="w-full text-sm">
            <thead className="bg-slate-950/60 border-b border-white/5">
              <tr>
                ...
                <th className="text-left px-4 py-3 text-xs font-semibold text-slate-400 uppercase">Workflow</th>
                <th className="text-left px-4 py-3 text-xs font-semibold text-slate-400 uppercase">Expiry</th>
            ...
            <tbody className="divide-y divide-white/5">
              {items.map(dt => (
                <tr key={dt.id} className="hover:bg-white/[0.02]">
                  <td className="px-4 py-3 text-white font-medium">{dt.name}</td>
                  <td className="px-4 py-3 text-slate-400 text-xs">{dt.workflow_template?.name || '—'}</td>
                  <td className="px-4 py-3 text-slate-400 text-xs">{dt.expiry_days ? `${dt.expiry_days} days` : 'Never'}</td>
