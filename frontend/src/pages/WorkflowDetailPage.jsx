...
export default function WorkflowDetailPage() {
  ...
  return (
    <div className="max-w-4xl mx-auto space-y-6">
      {workflowQuery.isLoading && <p className="text-sm text-[var(--th-loading-text)] py-8 text-center">Loading…</p>}
        ...
        <>
          <div className="flex items-start justify-between">
            ...
            <div className="flex items-center gap-3">
              <Link to={`/workflows/${id}/edit`} className="px-4 py-2 text-xs font-medium text-[var(--th-btn-primary-text)] bg-[var(--th-btn-primary-bg)] border border-[var(--th-btn-primary-border)] hover:bg-[var(--th-btn-primary-hover)] rounded-lg">Edit</Link>
              ...
              {steps.map((step, i) => (
                <div key={step.id} className="flex gap-4">
                  ...
                  <div className="flex flex-col items-center">
                    <div className="w-8 h-8 rounded-full bg-[var(--th-step-num-bg)] border border-[var(--th-step-num-border)] flex items-center justify-center text-xs font-bold text-[var(--th-step-num-text)] flex-shrink-0">
                      {step.step_order}
                    </div>
